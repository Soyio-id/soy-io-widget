...
<script>
  ...
  const privacyCenterOptions = {
    ...
    // When omitted, the DSR form starts with a right selection step.
    enabledRights: ["arsop"],
...
- `enabledRights`: Optional array of rights for the Data Subject Request (DSR) form. Supported values: `"arsop"`, `"redec"`. When omitted, the form starts with a right selection step. When a single value is provided, the selection step is skipped. When multiple values are provided, the selection step renders with only those options.
